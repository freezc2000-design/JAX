# GASAura / Y3demo — Codex operating notes
...
The bundled **UE_MCP_Bridge** plugin (v0.3.0, `Plugins/UE_MCP_Bridge/`) runs a JSON-RPC WebSocket server on **port 9877** whenever the editor is open. The matching MCP server (`ue-mcp` Node package) is registered in `.mcp.json` (project scope) — Codex launched in this directory will pick it up automatically.
...
- **Don't install a second MCP plugin alongside `UE_MCP_Bridge`.** On 2026-05-18 a plugin called `UnrealMCP` (chongdashu/unreal-mcp lineage) was added and broke the Editor's compile because:
  ...
  - **If you're tempted by VibeUE / cwilcox0916/Codex-ue-plugin** — they have advantages (in-editor chat panel, 953 methods, deeper Codex integration) but **not enough to justify a full migration mid-project**. Wait until current `UE_MCP_Bridge` hits a real feature wall.
