...
AuraDamageStatics::AuraDamageStatics()
{
	...
	DEFINE_ATTRIBUTE_CAPTUREDEF(UAuraAttributeSet, AttackPower, Source, false)   // 攻击力(来源)
...
UExecCalc_Damage::UExecCalc_Damage()
{
	...
	RelevantAttributesToCapture.Add(GetDamageStatics().AttackPowerDef);   // 攻击力
}
	...
	FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
	...
	// 攻击力:来源攻击力直接放大技能基础伤害 —— 基础值 ×(1 + 攻击力/100)。
	// 攻击力是可直接增减的二级属性(强化卡/成长直接加);攻击力=0(如怪)→×1 兼容。
	// 例:攻击力50 → 伤害×1.5。
	{
		float SourceAttackPower = 0.f;
		ExecutionParams.AttemptCalculateCapturedAttributeMagnitude(GetDamageStatics().AttackPowerDef, EvaluationParameters, SourceAttackPower);
		SourceAttackPower = FMath::Max(0.f, SourceAttackPower);
		Damage *= (1.f + SourceAttackPower * 0.01f);   // 攻击力1点 = +1%伤害
	}
